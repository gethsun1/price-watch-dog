...
export function parseAndValidateWorkflow(args: {
  yamlPath?: string;
  yamlContent?: string;
}): ValidationResult {
  ...
  if (!wf.nodes || typeof wf.nodes !== "object") {
    errors.push("workflow missing nodes");
  }

  // Required pipeline nodes by spec.
  const requiredNodes = ["fetch", "verify", "compare", "return", "deliver"];
  for (const n of requiredNodes) {
    if (!wf.nodes?.[n]) errors.push(`missing required node: ${n}`);
  }

  // Validate each node definition.
  if (wf.nodes) {
    for (const [nodeName, nodeDef] of Object.entries(wf.nodes)) {
      if (!nodeDef?.module) errors.push(`node ${nodeName} missing module`);
      if (!nodeDef?.function) errors.push(`node ${nodeName} missing function`);

      if (nodeDef.inputs && !Array.isArray(nodeDef.inputs)) {
        errors.push(`node ${nodeName} inputs must be an array`);
      }
      if (nodeDef.outputs && !Array.isArray(nodeDef.outputs)) {
        errors.push(`node ${nodeName} outputs must be an array`);
      }
    }
  }

  // Validate edges exist and are acyclic and connect known nodes.
  const edges = wf.edges ?? [];
  for (const e of edges) {
    if (!wf.nodes?.[e.from]) errors.push(`edge from unknown node: ${e.from}`);
    if (!wf.nodes?.[e.to]) errors.push(`edge to unknown node: ${e.to}`);
  }

  // Enforce the expected linear chain (spec requirement).
  const expectedEdges = [
  ...
  // Basic I/O name connectivity checks for this workflow flavor.
...
export async function executeWorkflowDAG(args: {
  ...
  for (const nodeName of order) {
    ...
    let out: any;
    if (nodeName === "deliver") {
      // deliver kernel expects a single object input
      out = await handler({
        result: ctx.result,
        chains: ctx.chains,
    ...
    // Store by declared output name when present, otherwise by node name.
    const outKey = node.outputs?.[0] ?? nodeName;
    ctx[outKey] = out;

    // Convenience aliases expected by downstream nodes in this pipeline.
